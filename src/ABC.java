	...
	public void run(){
		...
		for(cycle=0; cycle<maxCycle && maxFit!=1; cycle++){
			...
			for(int i=0; i<bee.length && maxFit!=1; i++){
                ...
                if (bee[i].getFitness() > oldFitness) System.out.println("    -> IMPROVED Fitness to: " + String.format("%.5f", bee[i].getFitness()));
			...
			for(int i=0; i<bee.length && maxFit!=1; i++){
                ...
                if (maxOnlooker > 0) System.out.println("  [Onlooker Bees] Visiting Bee " + i + " (" + maxOnlooker + " times)");
                ...
                for(int count=0; count<maxOnlooker; count++){
                    ...
                    if (bee[i].getFitness() > oldFitness) System.out.println("    -> Onlooker IMPROVED Fitness to: " + String.format("%.5f", bee[i].getFitness()));
